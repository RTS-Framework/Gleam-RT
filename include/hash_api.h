...
// HASHAPI_ORDINAL is the special procedure hash of the ordinal mode, it
// makes FindAPI look up the export by ordinal instead of by name, and it
// is supported by all of the FindAPI entry points.
//
// When it is used, the argument key carries the target function ordinal.
// The ordinal is the ordinal in the target module itself, it is not the
// index of the export address table. It returns NULL when the module is
...
//     argument keeps its meaning (module / selector)
//   - the same value is the salt of the module name hash, so the module
//     hash of an ordinal lookup is derived from the ordinal itself, a
//     mismatched module hash / ordinal pair fails to resolve instead of
//     resolving to another procedure
//   - the module hash and the ordinal can be generated by one tool call
//     (-key <ordinal>), the printed { module, procedure, key } triple is
//     used as is
//
// Notice: the ordinal only has 16 bit of salt entropy, it binds the
// constants together, it is not a replacement of a random key.
